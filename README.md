...
- **Piggies** — one-glance tracker for every savings category ("piggy") and its subcategories ("piglets"): lifetime balance derived from deposits minus withdrawals (excluding savings assigned to a Capitals plan), an optional target and deadline with a progress ring, and a **Saving Insights** engine (savings rate, streak, best month, pace-vs-deadline projections) surfaced on both the Piggies and Insights views. Saving Insights spans **Piggies and Capitals together**: assigned deposits count toward the wallet-wide rate, net, streak, and best month (split out as piggies vs capitals) and every plan gets a pace line — set aside, left to save, monthly pace, and funded / on pace / behind / overpaid — while per-piggy pace stays capital-free. Linked from Overview, Budgets, and Categories; exports to its own CSV
- **Capitals** — planner for big future expenses (marriage, trips, car/house loans, or a custom plan): start from a template or blank, set a **total budget**, check off line items as paid, see **Unspent** from savings deposits optionally assigned to the plan, a **monthly save** hint of (budget − paid − unspent) ÷ months until the target (or **Overpaid** when paid exceeds budget), **Total Unspent** and **Monthly Saving** summaries across plans, and **Log** a real payment straight into a prefilled expense that links back to the item
