...
- **Capitals** — planner for big future expenses (marriage, trips, car/house loans, or a custom plan): start from a template or blank, set a **total budget**, check off line items as paid, see a **monthly save** hint of (budget − paid) ÷ months until the target (or **Overpaid** when paid exceeds budget), a **Monthly Saving** summary across plans, and **Log** a real payment straight into a prefilled expense that links back to the item
